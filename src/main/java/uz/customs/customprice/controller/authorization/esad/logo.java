...
import uz.customs.customprice.config.security.SpringSecurityAuditorAware;
...
public class logo extends HttpServlet {
    ...
    public static Connection getConCPID(HttpServletRequest request) throws Exception {
        ...
        String strUrl = "jdbc:as400:" + strDB + "/CPID;date format=iso";
        Connection ConST = null;
        String classforName = "com.ibm.as400.access.AS400JDBCDriver";
        Class.forName(classforName).newInstance();
        ConST = DriverManager.getConnection(strUrl, strUser, strPwd);
...
//        String UUser_Id = up.getUUser_Id();
        ...
        if (userLogin != null) {
            ...
            Map<String, String> INN = new HashMap<String, String>();
            Map<String, String> PNFL = new HashMap<String, String>();
            boolean acces = true;
            ...
            try {
                if (acces) {
                    ...
                    String zapros = " select\n" +
                            ...
                            "on\n" +
                            "    usr.id=rs.role_id\n" +
                            "and usr.sysid='S053' /*expertise test*/\n" +
                            "join\n" +
                            ...
                            "    um.id = eu.user_id\n" +
                            "left join\n" +
                            "    unicon.certificates c\n" +
                            "on\n" +
                            "    eu.serial_number = c.issuer_serial_number\n" +
                            "where\n" +
                            "    um.isdeleted=0\n" +
...
//                        USERID.put(Integer.toString(t), (Utils.nullClear(rslt.getString("userid"))));
                        ...
                        INN.put(Integer.toString(t), (Utils.nullClear(rslt.getString("inn"))));
                        PNFL.put(Integer.toString(t), (Utils.nullClear(rslt.getString("pnfl"))));
                    }
...
//                    String sqlstr = "insert into cpid.log(ses_id, USER_ID, name, ip, enter, last) " + "values ('" + sessionID + "','" + up.getUUser_Id().substring(2) + "','" + up.getUName() +
//                    "','" + IP + "',current_timestamp,current_timestamp)";
                    String sqlstr = "insert into cpid.log(ses_id, USER_ID, name, ip, enter, last) " + "values ('" + sessionID + "','" + up.getUId() + "','" + up.getUName() + "','" + IP + "'," +
                    ...
                    String inn = INN.get("1");
                    String pnfl = PNFL.get("1");
                    ...
                    while (rslt2.next()) {
                        ...
                        inn = Utils.nullClear(rslt2.getString("inn"));
                        pnfl = Utils.nullClear(rslt2.getString("pnfl"));
                    }
                    rslt2.close();
                    User user = new User();
                    if (id.equals("")) {
                        UUID uuid = UUID.randomUUID();
                        PreparedStatement USER = conET.prepareStatement("insert into cpid.user_e (id, userid, username, fullname, password, location, locationname, post, postname, lang, notification, insuser, isdeleted, role, instime, inn, pnfl) \n" + "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                        ...
                        USER.setString(16, inn);
                        USER.setString(17, pnfl);
                        USER.execute();
                        USER.close();
